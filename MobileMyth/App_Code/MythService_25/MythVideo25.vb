...
Public Class MythVideo25
    ...
    Public Function GetVideo(Id As Integer) As iMythVideo.VideoMetadataInfo Implements iMythVideo.GetVideo
        Dim Vid As MythVideo.VideoMetadataInfo = Video.GetVideo(Id)

        Dim retvid As iMythVideo.VideoMetadataInfo = Common.ConvertTypes(Of MythVideo.VideoMetadataInfo, iMythVideo.VideoMetadataInfo)(Vid)
        retvid.Artwork = New List(Of iMythVideo.ArtworkInfo)

        For Each art As MythVideo.ArtworkInfo In Vid.Artwork.ArtworkInfos
            retvid.Artwork.Add(Common.ConvertTypes(Of MythVideo.ArtworkInfo, iMythVideo.ArtworkInfo)(art))
        Next
    ...
    Public Function GetVideoList(Descending As Boolean, StartIndex As Integer, Count As Integer) As List(Of iMythVideo.VideoMetadataInfo) Implements iMythVideo.GetVideoList
        ...
        For Each vid As MythVideo.VideoMetadataInfo In Video.GetVideoList(Descending, StartIndex, Count).VideoMetadataInfos
            Dim retvid As iMythVideo.VideoMetadataInfo = Common.ConvertTypes(Of MythVideo.VideoMetadataInfo, iMythVideo.VideoMetadataInfo)(vid)
